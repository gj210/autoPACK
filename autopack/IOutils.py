...
def save_asJson(env,setupfile,useXref=True):
        ...
        env.jsondic = {"recipe":{"name":env.name,"version":env.version}}
        env.jsondic["options"]={}
        ...
        if r :
            ...
            for ingr in r.ingredients:                
                if useXref :
                    ...
                    ing_filename = autopack.revertOnePath(pathout+os.sep+ingr.o_name+".json")
                    env.jsondic["cytoplasme"]["ingredients"][ingr.name]={"name":ingr.o_name,"filename":ing_filename)}
                else :
                    env.jsondic["cytoplasme"]["ingredients"][ingr.name]={"name":ingr.o_name}
                    for k in ingr.KWDS:
                        v = getattr(ingr,k)
    #                    print ingr.name+" keyword ",k,v
                        env.jsondic["cytoplasme"]["ingredients"][ingr.name].update(setValueToJsonNode(v,k)) 
        if len(env.compartments):
            env.jsondic["compartments"]={}
        for o in env.compartments:
            env.jsondic["compartments"][str(o.name)]={"geom":str(o.filename),#should point to the used filename
                                        "rep":str(o.representation),
                                        }#None
            if o.representation is not None :
                fileName, fileExtension = os.path.splitext(o.representation_file)
            else :
                fileName = None
            env.jsondic["compartments"][str(o.name)]["rep_file"]=str(fileName)#None
            rs = o.surfaceRecipe
            if rs :
                ...
                for ingr in rs.ingredients: 
                    if useXref :
                        #write the json file for this ingredient
                        io_ingr.write(ingr,pathout+os.sep+ingr.name,ingr_format="json")
                        #use reference file
                        env.jsondic["compartments"][str(o.name)]["surface"]["ingredients"][ingr.name]={"name":ingr.o_name,"filename":str(pathout+os.sep+ingr.name+".json")}
                    else :
                        env.jsondic["compartments"][str(o.name)]["surface"]["ingredients"][ingr.name]={"name":ingr.o_name}
                        for k in ingr.KWDS:
                            v = getattr(ingr,k)
        #                    print ingr.name+" keyword ",k,v
                            env.jsondic["compartments"][str(o.name)]["surface"]["ingredients"][ingr.name].update(setValueToJsonNode(v,k)) 
            ...
            if ri :
                ...
                for ingr in rs.ingredients: 
                    if useXref :
                        ...
                        io_ingr.write(ingr,pathout+os.sep+ingr.name,ingr_format="json")
                        #use reference file
                        env.jsondic["compartments"][str(o.name)]["interior"]["ingredients"][ingr.name]={"name":ingr.o_name,"filename":str(pathout+os.sep+ingr.name+".json")}
                    else :
                        env.jsondic["compartments"][str(o.name)]["interior"]["ingredients"][ingr.name]={"name":ingr.o_name}
                        for k in ingr.KWDS:
                            v = getattr(ingr,k)
        #                    print ingr.name+" keyword ",k,v
                            env.jsondic["compartments"][str(o.name)]["interior"]["ingredients"][ingr.name].update(setValueToJsonNode(v,k)) 
        with open(setupfile+".json", 'w') as fp :#doesnt work with symbol link ?
            json.dump(env.jsondic,fp,indent=1,separators=(',', ':'))#,indent=4, separators=(',', ': ')
        print ("recipe saved to ",setupfile+".json")
...
def load_Json(env,setupfile):
    ...
    with open('myTest.json', 'r') as fp :#doesnt work with symbol link ?
        env.jsondic=json.load(fp)#,indent=4, separators=(',', ': ')
