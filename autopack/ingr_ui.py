...
class SphereTreeUI(uiadaptor):
    ...
    def initWidget(self, id=0):
        ...
        self.BTN["display_distance"] = self._addElemt(name="display_distance",
                                                      ...
                                                      action=self.displayGridAsSpheres,
                                                      label="Disply grid distance as scaled spheres",
                                                      variable=self.addVariable("int", 0))
        self.BTN["display_distance_cube"] = self._addElemt(name="display_distance_cube",
                                                           width=100, height=10, type="button", icon=None,
                                                           action=self.displayGridAsCubes,
                                                           label="Disply grid distance as colored cubes",
                                                           variable=self.addVariable("int", 0))
        ...
        self.LABELS["nbS"] = self._addElemt(label="Number of clusters :", width=100)
        ...
        self.LABELS["ei"] = self._addElemt(label="B ond cutoff", width=100)
        self.IN = {}
    ...
    def setupLayout(self):
        ...
        frame = self._addLayout(id=196, name="SphereTree Build", elems=self.framesphereTree,
                                collapse=False)  # , type="tab")#,type="tab")
        ...
        frame = self._addLayout(id=196, name="Grid Build", elems=self.frameGridify,
                                collapse=False)  # , type="tab")#,type="tab")
    ...
    def displayClustersSpheres(self, clusters, factor=0.7):
        ...
        if not self.clusterCenterSpheres:
            self.clusterCenterSpheres = self.helper.instancesSphere("clcspheres",
                                                                    ...
                                                                    self.baseSphere, colors[:nbc], self.sc,
    ...
    def axe(self, i, c, d, axe, color):
        ...
        if cyl is None:
            cyl = self.helper.oneCylinder("clcyl" + str(i) + str(axe), head, tail, radius=1., instance=pinstance,
                                          material=None,
    ...
    def displayClustersBonds(self, clusters, factor=0.7, ev=1.0):
        ...
        for i, pair in enumerate(pairs):
            ...
            cl1.encapsualtingRadius() - cl1.radiusOfGyration())  # or diameter ?
            pinstance = self.helper.getObject("BC")
            if pinstance is None:
                pinstance = self.helper.Cylinder("BC", radius=1., length=1., res=10, pos=[0., 0., 0.], parent=None)[0]
            cyl = self.helper.getObject("clbond" + str(i))
            if cyl is None:
                cyl = self.helper.oneCylinder("clbond" + str(i), head, tail, radius=rad, instance=pinstance,
                                              material=None,
                                              parent=self.CenterSpheres, color=colors[nbc])
            else:
    ...
    def getCurrentSelectedPoints(self):
        ...
        if self.object_target is None:
            ...
            print (str(len(sel)) + " objects selected")
            if len(sel) == 1:
                self.object_target = sel[0]
                self.object_target_name = self.helper.getName(self.object_target)
                # type of seletcted object ?
                for s in sel:
                    s = self.helper.getMesh(s)
                    print ("type", self.helper.getType(s), self.helper.MESH, self.helper.POLYGON)
                if self.helper.getType(sel[0]) == self.helper.POLYGON or self.helper.getType(
                    ...
                    print ("found pts ", len(points), sel[0])
            else:
    ...
    def displayGridAsSpheres(self, *args, **kw):
        ...
        sphs = self.helper.instancesSphere(self.object_target_name + "distances", positions, distances, base, colors,
                                           None, parent=p)
