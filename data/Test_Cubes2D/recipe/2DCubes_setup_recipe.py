...
if forcedPriority:
    d={}
    d["5_n200"] =   {"rad":200,"dic":   {"packingPriority":-50,"nbMol":4,"color": rgbWhite}}
    d["10_n100"] =  {"rad":100,"dic":   {"packingPriority":-40,"nbMol":8,"color": rgbGrey}}
    d["200_n50"] =  {"rad":50,"dic":    {"packingPriority":-30,"nbMol":16,"color": rgbBlue}}
    #d["400_n25"] = {"rad":25,"dic":    {"packingPriority":0,"nMol":60,"color":None}}
    d["x400_n25"] = {"rad":25,"dic":    {"packingPriority":-20,"nbMol":32,"color": rgbPurple}}
    d["y400_n25"] = {"rad":25,"dic":    {"packingPriority":-20,"nbMol":32,"materials": red}}
...
#    d["b400_n25"] = {"rad":25,"dic":    {"packingPriority":0,"nMol":16,"color":None}}
    ...
    d["5_n200"] =   {"rad":200,"dic":   {"packingPriority":0,"nbMol":4,"color": rgbGrey}}
    d["10_n100"] =  {"rad":100,"dic":   {"packingPriority":0,"nbMol":8,"color": rgbDarkGrey}}
    d["200_n50"] =  {"rad":50,"dic":    {"packingPriority":0,"nbMol":16,"color": rgbBlue}}
#    d["400_n25"] =  {"rad":25,"dic":    {"packingPriority":0,"nMol":60,"color":None}}
    d["x400_n25"] = {"rad":25,"dic":    {"packingPriority":0,"nbMol":32,"color": red}}
    d["y400_n25"] = {"rad":25,"dic":    {"packingPriority":0,"nbMol":132, "color": rgbPurple}}
#    d["a400_n25"] = {"rad":25,"dic":    {"packingPriority":0,"nMol":16,"color":None}}
#    d["b400_n25"] = {"rad":25,"dic":    {"packingPriority":0,"nMol":16,"color":None}}
...
#    if helper.host != "c":
    ...
    if m is None :
       m,mesh = helper.box(k,center=[0.,0.,0.],size=[d[k]["rad"],d[k]["rad"],d[k]["rad"]],parent=vBaseGeometryHider)
#       m,mesh = helper.Sphere(k,radius=d[k]["rad"],res=12,parent=vBaseGeometryHider)
       helper.toggleDisplay(m,showFillGeoms)	
#    else :
#       m = helper.getObject(k)
    ingr = SingleCubeIngr( MSca*.0,  radii = [[d[k]["rad"],d[k]["rad"],d[k]["rad"]],],name='i'+k, pdb=None,
                              positions=[[[0,0,0],[0,0,0],[0,0,0],]],
                                meshObject=m,placeType=mode,**d[k]["dic"])
    print (ingr.packingPriority)
    lingr[k]=ingr
...
#pad = 20.
#
#h1.boundingBox = [[0.,0.,0.],[1000.,1000.,6.]]
#bbox = afviewer.helper.getObject("histoVolBB")
#if bbox is None : bbox = afviewer.helper.box("histoVolBB", cornerPoints=[[-500.,-500,-6.],[500,500,6.]])#cornerPoints=h1.boundingBox)
...
h1.runTimeDisplay = 1 #interactive display
...
resultfilename = h1.resultfile =wrkDir+os.sep+"autoFillRecipeScripts/2DsphereFill/results/CubesfillResultfromHistoVol.afr"
...
#===============================================================================
try :
    AFGui.Set("Test_Cubes2D",helper=afviewer.helper,afviewer=afviewer,histoVol=h1,bbox=bbox)
except:
    print ("no GUI")
print (7,time()-tStart)
tStart = time()
...
def GRID(h,forceBuild=True, fill=0):
    t1 = time()
...
def FILL(h,seed=20,forceBuild=True):
    t1 = time()
    h.fill5(seedNum=seed,verbose=4, vTestid = testid)
    t2 = time()
